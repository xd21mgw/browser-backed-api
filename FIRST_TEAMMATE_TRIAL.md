...
看到 `action_count=70`，并且 `auth_state=ready` 或你要用的 origin ready，即服务正常。
...
- `profile in use`：先运行 `npm run worker:doctor`。如果是日常 Chrome，
  不要关闭；检查是否误把 `BROWSER_BACKED_PROFILE_DIR` 指向了日常 Chrome
  profile。如果是 browser-backed 专用 profile 残留窗口，请用户手动关闭那个专用窗口。
