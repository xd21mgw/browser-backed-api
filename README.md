...
- optional typed input: `sub_interface=getLastestDateTime|getUseDuration|profile`, `time_window`
- fixed params generated by the service: `product/type` query params for `getLastestDateTime`, `appName/funcType/_t/userId|deviceId` body fields for `getUseDuration`, or `appName/startTime/endTime/include/pageSize/funcType/_t/userId|deviceId` body fields for `profile`
- live output is shape-only; `getUseDuration` returns an `activity_summary`, and `profile` returns a `profile_summary`
