...
- optional typed input: `sub_interface=getLastestDateTime|getUseDuration`, `time_window`
- fixed params generated by the service: `product/type` query params for `getLastestDateTime`, or `appName/funcType/_t/userId|deviceId` body fields for `getUseDuration`
- live output is shape-only; `getUseDuration` also returns an `activity_summary` with row counts and duration aggregates
