...
`track_analysis_summary` currently uses the smallest verified track-analysis API shape:

- sub-interface: `getLastestDateTime`
- method/path: `GET /dp/platform/app/analytics/v2/sequence/getLastestDateTime`
- typed input: exactly one of `user_id` or `device_id`, plus `appName=KUAISHOU|NEBULA`
- fixed query params generated by the service: `product`, `type`, `funcType=USER_PROFILE_QUERY`, `_t`
