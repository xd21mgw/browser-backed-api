...
| `archives_user_profile` | Archives Center / `archives` | `GET` | `/archives/user/home/info` | `user_id` | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed path only; context request injects HAR-aligned Archives Referer/Origin contract and uses `keyword=<user_id>`. |
| `archives_user_analysis` | Archives Center / `archives` | `POST` | `/v3/user/log/coreLogs/fetch` | `user_id`, `beginTime`, `endTime`, optional page controls | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed request body from typed fields only; context request injects HAR-aligned Archives Referer/Origin contract. |
| `archives_photo_search` | Archives Center / `archives` | `POST` | `/v4/archives/report/photo/search` | `user_id`, `begin`, `end`, optional page/filter params | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed path and typed params only; context request injects HAR-aligned Archives Referer/Origin contract. |
...
| `archives_gallery_photo_list` | Archives Center / `archives` | `POST` | `/v3/user/gallery/photo/list` | `user_id`, optional `pageIndex`, `pageSize`, `filters` | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed gallery list body only; context request injects HAR-aligned Archives Referer/Origin contract. |
...
| `archives_related_users` | Archives Center / `archives` | `POST` | `/archives/user/search/device` | `user_id`, optional `relation_type` | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed relation enum only; context request injects HAR-aligned Archives Referer/Origin contract. |
| `archives_private_message_search` | Archives Center / `archives` | `POST` | `/archives/user/message/search` | `user_id`, `direction`, optional page/filter controls | passthrough only | bounded | yes | yes | `live_pass` | `open_explicit` | Fixed private-message search body; context request injects HAR-aligned Archives Referer/Origin contract. |
| `archives_past_four_items` | Archives Center / `archives` | `POST` | `/v4/audit/user/fourinfo/log/search` | `user_id`, optional info type/page/filter controls | passthrough only | bounded | yes | yes | `live_complete` | `open_explicit` | Fixed four-info search body; context request injects HAR-aligned Archives Referer/Origin contract. |
