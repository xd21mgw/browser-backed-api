...
For the verified low-approval runtime path, run `npm run worker:expose` after
the Mac service is ready. It prints the dynamic
`service_base_url=http://<mac_ip>:9787` that the remote main Agent should use
for daily action calls. Do not hardcode a previously observed IP.
...
POC succeeds, it can become a future focus area or a promoted deployment mode.

## Short-Term Bridge Options

The verified fallback and low-approval paths are recorded in
